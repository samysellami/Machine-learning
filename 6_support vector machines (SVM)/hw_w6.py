...
def linear_kernel(x, z):
    return  np.dot(x.T,z)*np.dot(x.T,z)
...
def polynomial_kernel(x, z):
    return (np.dot(x.T,z) +1)*(np.dot(x.T,z) +1)*(np.dot(x.T,z) +1)
...
def minimize_and_plot(X, Y, kernel, C, thresh):
    ...
    alpha_constr = scipy.optimize.LinearConstraint(np.identity(n), 0, C)  # todo write your code here (one-liner)
    # constraints sum of (alpha * y)
    alpha_y_constr = scipy.optimize.LinearConstraint(Y, 0, 0)  # todo write your code here (one-liner)
